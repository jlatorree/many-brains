...
Este proyecto usa un `hub.md` en la raíz como índice vivo de todos los
...
temáticos (carpetas modulares), y esos nodes son las fuentes con las que
...
1. **Una fuente de verdad por tema.** Cada tema tiene un solo node.
...
3. **Hub como mapa.** Cuando crees, muevas, renombres o modifiques un
   node o un derivado, actualiza de inmediato la fila correspondiente en
   `hub.md` (ruta, fecha, versión, estado) y la fecha de su cabecera.

4. **Derivados citan su node.** Todo derivado que generes (HTML, .docx,
   ...
   node(s) como fuente — nunca inventes contenido que debería venir del
   node.

5. **Conexiones explícitas en wikilinks.** Cada node cierra con una sección
   `## Conexiones` que declara con enlaces `[[...]]` de qué depende, qué alimenta
   y con qué se relaciona. Mantenla actualizada al crear o modificar nodes.
   Es tu mapa de navegación: al abrir un archivo, lee sus Conexiones para saber
   qué otros nodes leer antes de responder o generar un derivado.
...
- **Cristalizar.** Cuando una conversación produzca conocimiento que vale la
  ...
  guardar es suya. Si acepta, guárdalo como node temático en su carpeta
  ...
  hub y dale su sección `## Conexiones`.
...
- **Derivar.** Cuando el usuario pida un derivado a partir de nodes
  ...
  "Basado en..." y registra la fila en la tabla de derivados del hub.

- **Lint (cuando el usuario lo pida).** Haz un health-check del proyecto y
  reporta: nodes sin sección `## Conexiones`; wikilinks `[[...]]` que apuntan
  a archivos que no existen; derivados marcados `requiere refresh` desde hace
  tiempo; contradicciones entre nodes; conceptos mencionados varias veces
  que merecerían su propio node. Propón correcciones, no las apliques sin
  confirmar.

## Reglas operativas adicionales

- Al modificar un node, marca como `requiere refresh` en el hub todo
...
- Las CORE son extensibles: si aparece una necesidad transversal que no encaja
  ...
  con prefijo `_` y regístrala en la tabla de estructura del hub.
- Carpetas MODULARES: créalas por tema y con nombre descriptivo (sin prefijo `_`)
  ...
  Agrégalas a la tabla de estructura del hub.
- Solo se entra a `_historico/` para congelar la versión exacta de un node
...
- Si detectas que el hub quedó desactualizado respecto al estado real de
